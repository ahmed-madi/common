...
class EmployeeResource(BaseResource):
    ...
    def leave_balance_action(cls):
        ...
        def _action(name):
            ...
            if not name:
                frappe.throw(_("Employee ID (name) must be provided"), frappe.MandatoryError)

            if not frappe.db.exists("Employee", name):
                frappe.throw(_("The requested employee could not be found"), frappe.DoesNotExistError)
                
            employee = frappe.get_doc("Employee", name)
            if not frappe.has_permission("Employee", "read", employee, frappe.session.user, False):
                raise frappe.PermissionError(_("You do not have permission to access employee details"))
            ...
            if cint(for_annual) == 1:
                ...
                if not annual_leave:
                    frappe.throw(_("Data for Annual Leave Type is not available"), frappe.DoesNotExistError)
    ...
    def get_routes(cls):
        ...
        # Add custom route
        routes.append(
            Rule(f"{base_url}/<path:name>/leave-balance", methods=["GET"], endpoint=cls.leave_balance_action())
        )
        return routes

...
from common.api.controllers.company.organizational_chart import get_employee_structure, get_department_structure
...
def event_list():
    ...
    return document_list(doctype, LIST_FIELDS, force_fields=True, user_filters=filters, force_user_filters=force_filters)
...
def employee_structure():
    def build_organization_tree(parent=None, company=None, exclude_node=None):
        children = get_employee_structure(parent=parent, company=company, exclude_node=exclude_node)
        if len(children) == 0:
            return []
        for child in children:
            child.update({
                "children": build_organization_tree(parent=child.get("id"), company=company, exclude_node=exclude_node)
            })
        return children
    company = None
    parent = None
...
def department_structure():
    def build_organization_tree(parent=None, company=None, exclude_node=None):
        children = get_department_structure(parent=parent, company=company, exclude_node=exclude_node)
        if len(children) == 0:
            return []
        for child in children:
            child.update({

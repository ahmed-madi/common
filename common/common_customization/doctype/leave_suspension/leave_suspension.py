...
class LeaveSuspension(Document):
	...
	def validate_previous_records(self):
		...
		if len(prev) > 0:
			frappe.throw(_("Employee has suspension detected {0}").format(get_link_to_form("Leave Suspension", prev[0].name)), frappe.UniqueValidationError)
	...
	def create_leave_ledger_entry(self, leave, submit=True):
		...
		date_difference = (date_diff(leave.to_date, self.return_date) + 1) * (1 if submit else -1)
		...
		if leave_period:
			leave_allocation = self.get_existing_allocation(comp_leave_valid_from, leave)
			if leave_allocation:
			...
			msg = _("This suspension leave will be applicable from {0}.").format(comp_leave_valid_from)
			msg += " " + _(
		...
		leave.add_comment(text="Suspension Leave by employee in {} for {} Day(s)".format(get_link_to_form("Leave Suspension", self.name, "Application"), date_difference))

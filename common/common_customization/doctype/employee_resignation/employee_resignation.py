...
class EmployeeResignation(BaseHRDocument):
    ...
    def validate_notice_days(self):
        notice_days = cint(frappe.db.get_single_value("Company Policy", "resignation_notice_days"))
        if notice_days <= 0:
            return

        if date_diff(self.last_working_day, self.request_date) < notice_days:
            frappe.throw(_("Resignation notice date must be before {} days from Last Working Day").format(notice_days))

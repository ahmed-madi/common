...
from frappe.utils import cint, flt, time_diff_in_hours, time_diff_in_seconds, get_first_day, get_last_day, getdate
...
class EarlyLeaveApplication(BaseHRDocument):
    ...
    def validate_on_record_in_same_day(self):
        other_requests = frappe.get_all("Early Leave Application", filters={
        ...
        if len(other_requests) > 0:
            frappe.throw(_("You have already submitted an early request in this date {}.").format(self.exit_date))
    ...
    def validate_exit_date_time(self):
        ...
        employee_shift = get_employee_shift(self.employee, exit_datetime, True, 'forward')
        ...
        if cint(policy.do_not_permit_early_leave) == 1:
            if not employee_shift or employee_shift is None:
                frappe.throw(_("Application for early leave is not allowed without an assigned shift"))
        ...
        if flt(policy.early_exit_max_hours_per_day) > 0 and total_exit_hours > flt(policy.early_exit_max_hours_per_day):
            frappe.throw(_("You have exceeded the allowed exit hours for today. {} hours Allowed, {} requested").format(flt(policy.early_exit_max_hours_per_day, 2), flt(total_exit_hours, 2)))
        ...
        total_current_month = frappe.db.sql("""
                            ...
                            """.format(self.employee, getdate(start_month), getdate(end_month)))
        total_in_month = flt(total_exit_hours, 2)
        if total_current_month:
            total_in_month += flt(total_current_month[0][0])
        if flt(policy.early_exit_total_hours_allowed) > 0 and total_in_month > flt(policy.early_exit_total_hours_allowed):
            frappe.throw(_("You have exceeded the allowed exit hours for this month. {} hours Allowed, {} requested").format(flt(policy.early_exit_total_hours_allowed, 2), flt(total_in_month, 2)))

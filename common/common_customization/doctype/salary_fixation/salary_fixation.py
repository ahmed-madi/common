...
class SalaryFixation(BaseHRDocument):
    def validate(self):
        super().validate()
        self.validate_backdate_restriction("restrict_backdated_sfx", "sfx_w_roles", self.effective_date)
        self.validate_salary_mode()
    
    def validate_salary_mode(self):
        salary_mode_details = frappe.db.get_values("Employee", self.employee, ["salary_mode", "iban"], as_dict=True)
        if not salary_mode_details:
            frappe.throw(_("Salary mode for {} must be 'Bank'").format(get_link_to_form("Employee", self.employee, _("Employee"))))

...
class TrainingRequest(BaseHRDocument):
	...
	def validate_training_policy(self):
		training_allowed_before = cint(frappe.db.get_single_value("Company Policy", "training_allowed_before"))
		if training_allowed_before > 0 and date_diff(self.start_date, self.request_date) < training_allowed_before:
			frappe.throw(_("Training request must be before {} days from start").format(training_allowed_before), frappe.InvalidStatusError)

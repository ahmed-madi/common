...
class EducationAllowanceRequest(BaseHRDocument):
    ...
    def validate_requested_amount(self):
        ...
        sum = frappe.db.sql(
            """
                        ...
                        """.format(
                self.name, self.academic_year, self.employee
            )
        )
        if not sum:
            return
        ...
        if max_education_allowance > 0 and sum > max_education_allowance:
            frappe.throw(
                _(
                    "You have exceeded the maximum allowed allowance for year {} {} Maximum allowed amount {} and Total requested amount {}"
                ).format(
                    self.academic_year,
                    "<br>",
    ...
    def make_payment_entry(self, expense_claim):
        ...
        payment_entry = get_payment_entry_for_employee(
            expense_claim.doctype, expense_claim.name
        )
